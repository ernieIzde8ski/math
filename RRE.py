...
def _print_matrix(matrix: FormattedMatrix, format: bool = False) -> None:
    matrix = [
        [(i if (not isinstance(i, Fraction) or i.denominator != 1) else i.numerator) for i in row] for row in matrix
    ]
    print(*matrix, sep="\n", end="\n" * 3)
...
def display_rre_steps(_input: list[list[num]] | list[list[str]] | str | None = None) -> FormattedMatrix:
    ...
    if isinstance(_input, str):
        _input = [row.split(",") for row in _input.strip().split(";") if row]
...
if __name__ == "__main__":
    # The program can also be called with syntax like:
    # python rre.py 1,2,-3,-3 -2,3,2,-25 -1,2,-3,-11
    import sys

    display_rre_steps(";".join(sys.argv[1:]))
